...
program
    ...
    .option("-n, --owner-name <name>", "the name of the owner of the account, used to determine if a order is a gift")
...
function categoryNameToId(categoryName) {
    return lunchMoneyCategories.find((category) => !category.is_group && category.name === categoryName)?.id;
}
...
log.info(`Amazon transactions we can match: ${amazonTransactions.length} (out of ${allAmazonTransactions.length})`);
...
const lunchMoneyAmazonTransactionsWithRefunds = allLunchMoneyTransactions.filter(
// TODO do we need to any sort of regex matching here? More fancy matching?
(transaction) => transaction.payee === "Amazon");
const lunchMoneyAmazonTransactions = lunchMoneyAmazonTransactionsWithRefunds.filter(
...
const uncategorizedLunchMoneyAmazonTransactions = lunchMoneyAmazonTransactions.filter(
...
log.info(`Lunch Money transactions we can match: ${uncategorizedLunchMoneyAmazonTransactions.length} (out of ${allLunchMoneyTransactions.length})`);
function findMatchingLunchMoneyTransaction(uncategorizedTransaction, remainingAmazonTransactions) {
    const normalizedPaymentAmount = parseFloat(uncategorizedTransaction.amount).toFixed(2);
    ...
    const possibleMatches = remainingAmazonTransactions.filter((amazonTransaction) => 
    ...
    possibleMatches.sort((one, two) => Math.abs(dateFns.differenceInDays(parsedDate, new Date(one.date))) -
        Math.abs(dateFns.differenceInDays(parsedDate, new Date(two.date))));
    const matchingTransaction = possibleMatches[0];
    const matchingTransactionIndex = remainingAmazonTransactions.findIndex((txn) => txn.orderid.trim() === matchingTransaction.orderid.trim());
...
function orderIsGift(transaction) {
...
for (const uncategorizedLunchMoneyAmazonTransaction of lunchMoneyAmazonTransactions) {
    const matchingAmazonTransaction = findMatchingLunchMoneyTransaction(uncategorizedLunchMoneyAmazonTransaction, amazonTransactions);
    if (!matchingAmazonTransaction) {
        log.warn(`no match\t${uncategorizedLunchMoneyAmazonTransaction.id}\t${uncategorizedLunchMoneyAmazonTransaction.amount}\t${uncategorizedLunchMoneyAmazonTransaction.payee}\t${uncategorizedLunchMoneyAmazonTransaction.notes}\t${uncategorizedLunchMoneyAmazonTransaction.date}`);
        continue;
    }
    // TODO maybe allow for an overwrite option?
    if (uncategorizedLunchMoneyAmazonTransaction.category_id !== defaultCategoryId) {
        log.debug("already categorized, but matched. Skipping");
        continue;
    }
    if (uncategorizedLunchMoneyAmazonTransaction.is_group ||
        uncategorizedLunchMoneyAmazonTransaction.group_id) {
        log.warn(`skipping group transaction ${uncategorizedLunchMoneyAmazonTransaction.id}`);
        continue;
    }
    ...
    if (orderIsGift(matchingAmazonTransaction)) {
        ...
        const matchingKey = Object.keys(categoryRules).find((key) => matchingAmazonTransaction.categories.startsWith(key));
        if (matchingKey) {
            targetCategoryName = categoryRules[matchingKey];
        }
    }
    // null is actually printed, which is why we need ""
    const newNote = `#${matchingAmazonTransaction.orderid} ${uncategorizedLunchMoneyAmazonTransaction.notes || ""}`.trim();
    const shouldUpdateNote = !(uncategorizedLunchMoneyAmazonTransaction.notes || "").includes(matchingAmazonTransaction.orderid);
    if (!targetCategoryName) {
        log.info(`no rule match for\t${uncategorizedLunchMoneyAmazonTransaction.id}\t${matchingAmazonTransaction.categories}`);
        ...
        if (!options.dryRun && shouldUpdateNote) {
            const response = await lunchMoney.updateTransaction(uncategorizedLunchMoneyAmazonTransaction.id, {
                notes: newNote,
            });
            if (!response.updated) {
    ...
    log.debug(`match\t${matchingAmazonTransaction.date} : ${uncategorizedLunchMoneyAmazonTransaction.date} : ${uncategorizedLunchMoneyAmazonTransaction.amount} : ${matchingAmazonTransaction.total} : ${uncategorizedLunchMoneyAmazonTransaction.id}`);
    const newCategoryId = categoryNameToId(targetCategoryName);
    if (!newCategoryId) {
        log.error("invalid category name", targetCategoryName);
        continue;
    }
    log.info(`updating transaction ${uncategorizedLunchMoneyAmazonTransaction.id}`);
    log.debug("content of update", newNote, newCategoryId);
    if (!options.dryRun) {
        const updateOptions = { category_id: newCategoryId };
        if (shouldUpdateNote) {
            updateOptions.notes = newNote;
        }
        const response = await lunchMoney.updateTransaction(uncategorizedLunchMoneyAmazonTransaction.id, updateOptions);
        if (!response.updated) {
            log.error("failed to update transaction", response);
        }
        else {
